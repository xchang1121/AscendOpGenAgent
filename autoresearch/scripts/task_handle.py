...
Background
==========
Every script-level entry (engine/baseline.py, engine/pipeline.py,
engine/create_plan.py, resume.py, hooks/post_bash._handle_activation,
batch/run.py) used to compose its own sequence of state-store calls:
heal? consistency-check? claim ownership? load progress? read phase?
write history + state + clear intent? Each entry picked a different
order, and when state.json's schema grew (progress_initialized,
intent.json, owner sub-fields, expected_*) only the entries the
patch happened to touch learned about it. The recurring bug class
was always "X script wasn't going through the new facade for Y."

This module collapses that surface into one class: ``Task``.
