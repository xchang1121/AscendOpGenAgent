...
verify 失败时 ref 时延由 profile_base 在同进程下一段单独测得，与 verify 解耦，dashboard 顶栏始终显示 PyTorch baseline。Sticky baseline 写定（`baseline_metric` 与 `baseline_source=ref` 写入 state.json）后，后续轮的 phase 列表不再包含 `profile_base`。
...
| `.ar_state/state.json` | 单文件控制态：phase / owner / progress fields / pending_settle / 一致性 expected_* |
...
控制态单文件：`<task_dir>/.ar_state/state.json`（phase / owner /
progress / pending_settle / 一致性 expected_*）。Ownership 通过
`state.owner.session_id` 匹配 `CLAUDE_CODE_SESSION_ID`；当前会话的
task 由 `phase_machine.find_active_task_dir()` 通过扫描 `ar_tasks/` 解析。

```
hook_guard_bash (bash 之前)
  从 state.json 读取 phase，按 phase 表允许/拒绝 AR 脚本
  仅认裸 `scripts/...`（或 `./scripts/`）为 AR 调用；cwd 即 autoresearch/，
  带前缀的 `autoresearch/scripts/...` 既被判为非规范形式拒绝、在磁盘上也不存在
  blessed CLIs：pipeline / baseline / create_plan / parse_args / scaffold /
                resume / dashboard
  其余 `python scripts/*.py` 调用走 _LIBRARY_NOT_CLI（含 pointed hint）或
  generic "Unknown script" 拒绝

hook_post_bash (任意 bash 之后)
  检测 "AR_TASK_DIR=" → 通过 set_task_dir 写入 state.owner，
                       初次激活时 PhaseController.on_activation_ready → BASELINE
  检测 baseline.py    → workflow.run_baseline_init 已在子进程里推进 phase
...
hook_post_task (任意 Task 工具调用之后)
  bump state.last_touched（防止长跑 DIAGNOSE 子代理超过 heartbeat 窗口）
  DIAGNOSE artifact 校验

hook_stop_save (Claude 试图 Stop 时)
  FINISH → 允许；BASELINE+no-progress（gate 拒绝提交的 baseline-pending）→ 允许；
  其余 phase → 拒绝，要求继续主循环
```
