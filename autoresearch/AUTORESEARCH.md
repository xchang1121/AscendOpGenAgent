...
测评机准备一个 conda env（约定叫 `autoresearch`），里面装齐：

- Python ≥ 3.10、PyYAML
- torch、torch_npu（匹配你的 CANN 版本）、triton-ascend
- CANN：把 `set_env.sh` 塞进 conda 的
  `<env>/etc/conda/activate.d/cann.sh`，让 `conda activate` 同时拉起
  CANN；不想塞就在 env.sh 里多 source 一行 set_env.sh

dev 端只要 Python ≥ 3.10、PyYAML、Claude Code CLI；torch_npu / CANN
不需要（eval 都委托给测评机）。

### env.sh：一行激活

多个场景要在非交互 shell 里把 env 拉起 — tmux 非 login shell、
`ar_cli worker --remote-host` 远端启动、batch happy-path 的
`bash --login -c 'source env.sh && python ...'`。所有这些都 source 同一个
脚本，路径填到 `config.yaml: remote_worker.hosts.<alias>.env_script`。
...
验证：`source ~/env.sh && python -c "import torch_npu, triton"` 不抛错就行。

> env.sh 里**不要** `cd`（外部 ar_cli 已经 `cd repo_path`）、`exec`、`set -e`。
