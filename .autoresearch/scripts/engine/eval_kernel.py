...
def run_profile(target_cls, ref_mod, cases_cpu, init_inputs, device,
    ...
    """Per-case latency via skill benchmark.measure_single.

    Skill's measure_single returns latency in milliseconds; autoresearch's
    metric pipeline is in microseconds end-to-end (history.jsonl, plan.md,
    dashboard, eval_client._per_shape_floats). Convert ms -> us at the
    boundary.

    `mode` is "gen" (kernel) or "base" (PyTorch reference); used in the
    profiler trace dir name so the two passes produce distinct artifacts
    even when run back-to-back.
    """
    import math
    from utils.input_groups import describe_case as _describe_case
    ...
    for idx, case in enumerate(cases_cpu):
        ...
        try:
            ...
            inputs_dev = [x.to(device) if hasattr(x, "to") else x
                          for x in case]
            operators, latency_ms, _ = skill_benchmark.measure_single(
                model, inputs_dev, warmup, repeats,
                profile_name=f"{mode}_case{idx}",
                device=device,
            )
            if (latency_ms is None or latency_ms <= 0
                    or latency_ms == float("inf")):
                raise RuntimeError(
                    f"measure_single returned invalid ms={latency_ms!r}")
            avg_us = float(latency_ms) * 1000.0
            method = "profiler" if operators else "fallback"
        except Exception as e:
...
def main():
    ...
    # Load skill core functions (verify.run_single_case + benchmark.measure_single)
