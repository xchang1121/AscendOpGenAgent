...
Examples:
    ...
    # need their own AscendOpGenAgent checkout, ar_cli runs on both)
...
# Repo root is two levels up: scripts/ → .autoresearch/ → repo
...
def _load_remote_host_config(alias: str) -> Optional[dict]:
    ...
    cfg_path = REPO_ROOT / ".autoresearch" / "config.yaml"
...
def _build_remote_ar_cli_cmd(host_cfg: dict, ar_cli_args: list[str]) -> str:
    ...
    parts.append(
        f"{shlex.quote(python)} .autoresearch/scripts/ar_cli.py "
